/* ═══════════════════════════════════════════════════════════════
   ...
   POST /api/social/media          Admin  — upload une image OU une vidéo (mp4) → R2
   GET  /api/social/media/:file    Public — sert le média (fetché par FB/IG/Telegram)
...
const EXT_BY_MIME = {
  ...
  'video/mp4': 'mp4',                        // Phase 3 — vidéo / short
};
const VIDEO_MIMES     = new Set(['video/mp4']);
const MAX_IMAGE_BYTES = 8  * 1024 * 1024;   // 8 Mo
const MAX_VIDEO_BYTES = 50 * 1024 * 1024;   // 50 Mo — clips courts / short (Reels ≤90 s).
                                            // Tient en RAM worker (128 Mo) ; vidéo longue = URL présignée R2 plus tard.
...
export async function handleSocialMediaUpload(request, env) {
  ...
  if (buf.byteLength > maxBytes) {
    return err(isVideo ? 'Vidéo trop volumineuse (max 50 Mo — réservé aux clips courts / short)'
                       : 'Image trop volumineuse (max 8 Mo)', 413, origin);
  }
  ...
  return json({ success: true, url, type: isVideo ? 'video' : 'image', bytes: buf.byteLength }, 200, origin);
}
...
export async function handleSocialMediaServe(request, env, filename) {
  if (!/^[0-9a-f-]{8,}\.(jpg|png|webp|gif|mp4)$/i.test(filename || '')) {
