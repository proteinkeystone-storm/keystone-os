  ...
  return `
    ...
    <div class="sdqr-body">
      <aside class="sdqr-sidebar">
        ...
        <button class="sdqr-overview-btn" id="sdqr-overview-btn" title="Statistiques de tous tes QR" style="display:flex;align-items:center;gap:8px;width:100%;margin:0 12px 10px;width:calc(100% - 24px);padding:9px 11px;border:1px solid rgba(108,108,245,.45);background:rgba(108,108,245,.13);color:#c9cdf7;border-radius:9px;font-size:12.5px;font-weight:600;cursor:pointer">
...
function _ovChart(byDay) {
  ...
  if (!v.length) return `<div style="height:140px;display:flex;align-items:center;justify-content:center;color:#6f7c93;font-size:12px">Pas encore de scans sur la période</div>`;
...
function _ovBars(items) {
  if (!items || !items.length) return `<div style="color:#6f7c93;font-size:12px;padding:6px 0">Aucune donnée</div>`;
  const max = Math.max(...items.map(i => i.scans), 1);
  return items.map(i => `<div style="margin-bottom:11px"><div style="display:flex;justify-content:space-between;font-size:12.5px;color:#cfd6e6;margin-bottom:5px"><span>${_esc(i.key)}</span><span style="color:#fff;font-weight:600">${_ovNum(i.scans)}</span></div><div style="height:7px;border-radius:4px;background:#283250"><div style="height:7px;border-radius:4px;width:${Math.round(i.scans / max * 100)}%;background:#6c6cf5"></div></div></div>`).join('');
}

function _ovLeader(lb) {
  if (!lb || !lb.length) return `<div style="color:#6f7c93;font-size:12px;padding:6px 0">Aucun scan pour l'instant</div>`;
  const max = Math.max(...lb.map(i => i.scans), 1);
  const arrow = t => t === 'up' ? '▲' : (t === 'down' ? '▼' : '■');
  const acol  = t => t === 'up' ? '#2bbf80' : (t === 'down' ? '#e0667a' : '#8a96ad');
  return lb.map((i, idx) => `<div style="display:flex;align-items:center;gap:11px;padding:8px 0;border-bottom:0.5px solid #ffffff0f"><span style="width:16px;font-size:13px;font-weight:700;color:${idx === 0 ? '#c9a84c' : '#6f7c93'}">${idx + 1}</span><div style="flex:1;min-width:0"><div style="font-size:12.5px;color:#eef1f8;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;margin-bottom:5px">${_esc(i.name)}</div><div style="height:6px;border-radius:3px;background:#283250"><div style="height:6px;border-radius:3px;width:${Math.round(i.scans / max * 100)}%;background:#6c6cf5"></div></div></div><span style="font-size:12.5px;font-weight:600;color:#fff;width:46px;text-align:right">${_ovNum(i.scans)}</span><span style="font-size:13px;color:${acol(i.trend)}">${arrow(i.trend)}</span></div>`).join('');
}

function _ovWatch(watch) {
  if (!watch || !watch.length) return `<div style="color:#6f7c93;font-size:12px;padding:6px 0">Rien à signaler — tout va bien.</div>`;
  return watch.map(w => `<div style="display:flex;align-items:flex-start;gap:10px;padding:9px 0;border-bottom:0.5px solid #ffffff0f"><span style="width:8px;height:8px;border-radius:50%;margin-top:3px;background:${w.kind === 'warn' ? '#e0a23a' : '#6c6cf5'};flex:none"></span><div style="flex:1"><div style="font-size:12.5px;color:#eef1f8">${_esc(w.name)}</div><div style="font-size:11px;color:#8a96ad;margin-top:1px">${_esc(w.note)}</div></div></div>`).join('');
}
...
function _renderOverviewHtml(d, period) {
  ...
  const kpi = (label, val, sub, extra = '') => `<div style="background:#18223a;border:0.5px solid #ffffff12;border-radius:12px;padding:14px 16px"><div style="font-size:10.5px;letter-spacing:0.06em;color:#8a96ad;margin-bottom:7px">${label}</div><div style="display:flex;align-items:baseline;gap:8px"><div style="font-size:27px;font-weight:800;color:#f3f5fb;line-height:1">${val}</div>${extra}</div><div style="font-size:11px;color:#8a96ad;margin-top:5px">${sub}</div></div>`;
  return `
  <div style="background:#0e1626;border-radius:14px;padding:18px;font-family:inherit">
    <div style="display:flex;align-items:flex-start;flex-wrap:wrap;gap:12px;margin-bottom:16px">
      <div>
        <div style="font-size:19px;font-weight:700;color:#f3f5fb;letter-spacing:-0.01em">Vue d'ensemble</div>
        <div style="font-size:12.5px;color:#8a96ad;margin-top:2px">Tous tes QR · données souveraines, aucune donnée tierce</div>
      </div>
      <div style="margin-left:auto;display:flex;background:#18223a;border:0.5px solid #ffffff14;border-radius:9px;overflow:hidden">
    ...
    <div style="background:#141d31;border:0.5px solid #ffffff10;border-radius:14px;padding:15px 16px;margin-bottom:13px">
    ...
    <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(258px,1fr));gap:13px;margin-bottom:13px">
      <div style="background:#141d31;border:0.5px solid #ffffff10;border-radius:14px;padding:15px 16px">
