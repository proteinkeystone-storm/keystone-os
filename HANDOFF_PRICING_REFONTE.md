...
### 🧹 GRAND NETTOYAGE DU 25/07 AU SOIR — la licence témoin N'EXISTE PLUS
**La licence `fe75db11…` (axia.communication, Ghost Writer, témoin de l'enforcement) était née d'un paiement Stripe en MODE TEST** — découvert en voulant l'annuler : l'abonnement n'existait qu'en test (19 €/mois fictifs). Supprimée proprement le 25/07 au soir : client Stripe test supprimé par Stéphane, puis **5 lignes D1 purgées** (licences + licence_subscriptions + ai_usage semée + ai_credit_balance 1000 + **licence_emails — la FK `licence_emails.licence_key → licences.key` bloque tout DELETE de licence, la purger D'ABORD**). **Base finale : 4 licences, toutes légitimes** (démo publique, ADMIN, MAX, gratuit lepaulette). Conséquences :
- Les points « supprimer la ligne semée » et « ouvrir le plafond aux autres licences » ci-dessous sont **CLOS** (plus de ligne, personne à armer — les nouvelles ventes s'arment seules via P7).
- ⚠️ **DÉCOUVERTE : le webhook n'accepte plus AUCUN événement de test depuis la bascule live (P6).** L'annulation test du 25/07 n'est jamais arrivée en base (`stripe_events` muet) — un seul secret `KS_STRIPE_WEBHOOK_SECRET`, celui du live ⇒ signature test = 400 silencieux. **On ne peut donc plus tester un achat de bout en bout.** Pour re-tester avant septembre : accepter les 2 secrets (test + live, ~15 lignes) — la colonne `livemode` (commit `d354a2c`, dormante, défaut penche vers 1, existants à NULL) marquera alors les licences d'essai → ménage = `DELETE FROM licences WHERE livemode = 0`.
- ✅ **Hygiène Stripe FAITE (26/07)** : endpoint webhook du mode test supprimé (67 % d'erreur — la preuve visuelle du diagnostic signature) ; en mode **production**, il y avait TROIS endpoints vers la même URL worker — seul « Keystone OS » (0 % d'erreur) porte le bon secret, les deux doublons auto-nommés (`upbeat-finesse`, `creative-dream`, 100 % d'erreur chacun) ont été supprimés. **Il ne reste qu'UN endpoint live, sain.** ⚠️ Leçon : chaque endpoint Stripe a son PROPRE secret de signature — un endpoint dupliqué vers la même URL est mort-né tant que le worker ne connaît qu'un secret.

### Reste à faire (par ordre de valeur)
1. **Plafonner les conversations** — ✅ CLOS PAR LE NETTOYAGE (la témoin est supprimée ; toute nouvelle vente d'app publique s'arme seule, cf. P7). Historique conservé ci-dessous.
   - `enforce_ai_credits_v1 = 1` était posé sur la témoin (sac = Ghost Writer seul → **300 conversations/mois**).
...
3. **Recharge automatique (P3) — 🟢 CONSTRUITE ET DÉPLOYÉE (25/07), un seul chemin jamais exécuté.**
   - **Livré** : `lib/auto-reload.js` (décision pure, 5 verrous : opt-in, plafond-mur, consentement horodaté, anti-rafale 10 min, pause collante — 35 tests) · routes `GET/POST /api/ai-credits/auto-reload` + `/setup` (session Stripe `mode:'setup'`, carte SANS débit) + `/resume` · webhook `mode:'setup'` qui rattache la carte · **balayage cron 5 min** (`auto-reload-sweep.js`) : débit off-session AVANT crédit, idempotence à 3 niveaux (anti-rafale > pas du cron, Idempotency-Key par fenêtre de 10 min, payment_intent en PK du journal), échec → pause + e-mail, JAMAIS de retry en boucle.
   - **`POST /api/admin/auto-reload/run-now`** (admin) déclenche sans attendre le cron ; **`?simulate=1`** = répétition générale à ZÉRO effet qui examine TOUTES les configs et dit ce qu'un vrai passage ferait (banc 19 tests : fetch remplacé par une grenade — la simulation ne PEUT pas toucher le réseau).
   - **Reste UNE chose jamais exécutée : le débit off-session réel** (~0,39 € de frais Stripe non remboursables, mesurés). Tout le reste est prouvé au banc ou validé en réel (l'achat manuel de pack l'a été de bout en bout le 25/07 : page renommée, e-mail pré-rempli, webhook, jauge).
   - ✅ **LE DÉBIT OFF-SESSION A TOURNÉ — banc d'essai Stripe test, 26/07 nuit, ZÉRO euro.** Stéphane n'avait pas la trésorerie pour le débit réel → banc à double univers construit (commit `1d27cab`) et déroulé la nuit même sur une licence gratuite dédiée (`livemode=0`, protein.std+banc-stripe@gmail.com). **Les 3 issues prouvées en réel** (journal `ai_auto_reload_log`) : ① 23:00 `succeeded` — PaymentIntent off-session confirmé, +1 000 conversations, jauge 9 €/20 €, pas de double débit aux passages suivants ; ② 23:20 carte refusée → **pause collante `payment_failed`** + e-mail, zéro retry, zéro débit ; ③ 23:30 carte 3DS → **pause `authentication_required`** + e-mail « votre carte n'est pas en cause » — LE cas d'incertitude du off-session, désormais couvert : une banque qui exige une validation ne casse rien, ne facture rien, et le client sait quoi faire. Bonus prouvés : webhook à double secret, levée de pause automatique à la carte fraîche. **Trouvaille UX corrigée dans la foulée (`cc2daa1`, SW v5.28.404)** : AUCUN état de l'écran n'offrait « changer de carte » — l'écran de pause exigeait un geste impossible ; le bouton existe désormais (nominal + pause), et a été validé en réel pendant le banc. Le banc reste en place, désarmé (`enabled=0`, licence conservée). Le débit sur carte RÉELLE reste optionnel — le code est identique, seule la clé diffère.
   - ⚠️ Pièges appris : pas d'écran de réglages front encore (console seulement) · une licence avec ≥ 50 conversations restantes répond `above_threshold` (le solde de packs compte) · **le banc du sweep s'est fait prendre au piège `PRICING_V2` absent de l'env de test** → quota du plan technique au lieu du sac.
4. **Renommer les 2 packs « conversations »** — ✅ **FAIT par Stéphane (25/07)** dans le tableau de bord Stripe, vérifié sur la page de paiement réelle. Prix intacts (9/39 €, le repli par montant en dépend).
5. **Annuel — ✅ TRANCHÉ (25/07) : PAS d'offre annuelle.** « Les 3 applications gratuites suffisent » comme geste commercial. La promesse « 2 mois offerts » retirée de la carte OS de la landing. La plomberie `interval:'year'` (serveur + `ANNUAL_MONTHS_BILLED`) reste dormante : elle ne promet rien à personne, l'arracher n'apporterait que du risque. Ne PAS re-proposer l'annuel.
6. **Palier gratuit — ✅ VALIDÉ DE BOUT EN BOUT (25/07)** : vraie inscription (`8VEF-…`, lepaulette.fr@gmail.com), sac = les 3 gratuites, **e-mail Resend reçu**, onboarding OK. 5 licences en base désormais.
7. ✅ **BOUCLÉ (26/07, commit `092d44b`, worker `8031d591`)** : un pack remboursé **rend ses conversations**. Handler `charge.refunded` : résolution dans le même ordre que l'achat (journal `ai_auto_reload_log` par payment_intent pour les recharges auto, sinon montant→pack + customer/e-mail), reprise PLANCHÉE au solde restant (jamais négatif — le déjà-consommé ne mord pas sur l'inclus). Strict par choix : remboursement PARTIEL → zéro reprise (l'admin tranche) ; montant d'ABONNEMENT → jamais de reprise (les droits d'abo partent par la résiliation). Banc 66/66. **Stéphane a ajouté `charge.refunded` à l'endpoint live (4 événements écoutés)**. ⚠️ Jamais vu tourner sur un vrai remboursement — à observer au premier.
8. ✅ **`BYOK_ROUTING = "on"` (26/07, même commit).** Flip fait au moment le plus sûr : coffre `tenant_api_keys` VIDE (vérifié) → le routage vendor ne s'appliquait à personne, et les surfaces publiques restent gardées par le mode déclaré per-app (P7, défaut MANAGED). Effet réel : l'option « Votre propre moteur » du panneau P7 devient atteignable dès qu'un client dépose une clé. Smoke prod 20 VERT après flip. ⚠️ La DÉGRADATION (clé morte → repli compté + e-mail) reste jamais vue en réel — à surveiller au premier client BYOK.
