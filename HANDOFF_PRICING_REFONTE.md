...
3. **Recharge automatique (P3) — 🟢 CONSTRUITE ET DÉPLOYÉE (25/07), un seul chemin jamais exécuté.**
   ...
   - 🔴 **TRANCHÉ 26/07 : Stéphane DIFFÈRE le débit réel — ne pas re-proposer, il le déclenchera lui-même.** Le plan reste prêt (licence gratuite lepaulette `8VEF-…` : quota 0 → restant sous le seuil dès l'armement ; ADMIN/MAX inéligibles → verdict `unlimited`). ⚠️ À ne pas perdre : l'achat manuel de pack et le off-session ne partagent PAS le même chemin Stripe — le off-session = PaymentIntent sur carte enregistrée SANS humain devant l'écran, avec le risque 3DS (la banque peut exiger une validation que personne n'est là pour donner). Le banc et la simulation couvrent tout SAUF cette réponse réelle de la banque. À exécuter une fois avant de laisser un vrai client compter dessus (idéalement avant sept.).
