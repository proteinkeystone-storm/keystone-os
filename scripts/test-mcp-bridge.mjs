...
console.log('\n▶ 1 · Canal SSE');
{
  ...
  eq(DB._db.prepare('SELECT args_json FROM mcp_jobs WHERE id = ?').get(jA).args_json, '{}', 'arguments effacés dès la remise à l’onglet');
...
console.log('\n▶ 3 · bridgeRun (attente du Worker)');
{
  ...
  const kept = DB._db.prepare('SELECT args_json, result_json, error FROM mcp_jobs WHERE id = ?').get(r.jobId);
  yes(kept.args_json === '{}' && kept.result_json === null && kept.error === null, 'confidentialité : arguments et résultat effacés de la base dès la lecture');
  ...
  eq(DB._db.prepare('SELECT args_json FROM mcp_jobs WHERE id = ?').get(r.jobId).args_json, '{}', '… arguments effacés à l’expiration');
