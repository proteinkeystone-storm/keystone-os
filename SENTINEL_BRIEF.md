...
| **S12-S14 — Promesses tenues · Lire le web · GEO vrai** ✅ déployés 2026-08-04 | Issus du RÉQUISITOIRE (exercice détracteur, 8 attaques) | S12 : gain réel par finding, fin du yo-yo échantillon/complet, reprise CWV étiquetée, axes renommés + pondérations au PDF. S13 : HTML visible (commentaires/scripts hors comptage), coquille SPA → indéterminé, NAP +32/+41/+352, squarespace/shopify managés. S14 : historique GEO lissé, **présence dans les sources citées** (le levier concret) + pont FAQ honnête. Règle permanente : passe adverse en clôture de chaque sprint, attaques → fixtures (46 tests). |
...
| **S16 — Ce que l'usage réel trouve** ✅ déployé 2026-08-04 | Le dogfooding comme source de défauts, pas comme formalité | Moteur pur passé hors prod sur **12 sites réels** (8 WordPress français, 8 Squarespace) — les deux plateformes n'avaient jamais été éprouvées en vrai. **6 bugs, zéro trouvé par le harnais de 50 tests.** **16.0** NAP lu sur le TEXTE et non sur le balisage (un attribut n'est pas du texte) : téléphone français au format national et adresse « place … CP commune » enfin vus ; coordonnée SVG et nom de fichier ne comptent plus — `mirobolus.fr`, vraie TPE au NAP complet, passait présence 0/100. **16.1/C24** fusion d'entités : deux fiches du même établissement sans `@id` commun → `jsonld_entity_split` (informatif), correctif avec le chemin Wix réel (« Convertir en balisage personnalisé ») — la promesse faite au client du Mas, tenue. **16.2** la plateforme se déduit d'une signature, pas d'un hasard : les jetons base64 aléatoires de Squarespace faisaient classer « Wix » une fois sur ~500, à vie (décidée une seule fois à la création), et un site sur-mesure mal étiqueté gagnait 60 pts de sécurité indus — désormais signatures + auto-réparation à chaque audit. 70 tests. |
